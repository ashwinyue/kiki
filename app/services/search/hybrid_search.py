...
class VectorSearcher:
    ...
    async def search(
        ...
        使用 pgvector 的余弦距离搜索，支持 HNSW 索引优化。
        ...
        if params.knowledge_ids:
            where_clauses.append("e.knowledge_id = ANY(:knowledge_ids)")
            params_dict["knowledge_ids"] = params.knowledge_ids

        # 应用标签过滤
        if params.tag_ids:
            where_clauses.append("e.tag_id = ANY(:tag_ids)")
            params_dict["tag_ids"] = params.tag_ids
        ...
        # 应用距离阈值过滤
        if params.vector_threshold > 0:
            # 余弦距离 = 1 - 余弦相似度
            distance_threshold = 1 - params.vector_threshold
        ...
        try:
            result = await self.session.execute(text(search_sql), params_dict)
            rows = result.fetchall()

            results = []
            for row in rows:
                score = float(row.score) if row.score else 0.0
                if score >= params.vector_threshold:
                    results.append((
                        row.chunk_id,
                        score,
                        row.content,
