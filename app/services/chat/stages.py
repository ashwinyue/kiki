...
class FilterTopKStage:
    ...
    对齐 WeKnora99 PluginFilterTopK

    根据 RerankTopK 配置过滤搜索结果数量。
    """
    ...
    def __init__(self, top_k: int | None = None):
        ...
        Args:
            top_k: 最大结果数，None 表示使用配置中的 rerank_top_k
        """
        self._top_k = top_k
    ...
    async def process(self, ctx: ChatContext) -> None:
        ...
        优先使用 rerank_results，如果没有则使用 search_results。
        """
        # 确定结果来源
        results = ctx.rerank_results or ctx.search_results

        if not results:
            logger.info(
                "filter_top_k_skipped",
                session_id=ctx.config.session_id,
        ...
        # 确定 Top-K
        top_k = self._top_k if self._top_k is not None else ctx.config.rerank_top_k

        if top_k <= 0:
            logger.info(
                "filter_top_k_skipped",
                session_id=ctx.config.session_id,
                reason="top_k_disabled",
            )
            return
        ...
        if len(results) > top_k:
            ...
            if ctx.rerank_results:
                ctx.rerank_results = filtered
            else:
                ctx.search_results = filtered
        else:
            logger.info(
                "filter_top_k_output",
                session_id=ctx.config.session_id,
...
class MergeStage:
    ...
    def __init__(self, min_content_length: int = 350):
        """初始化阶段

        Args:
            min_content_length: 最小内容长度，不足时尝试合并
        """
    ...
    async def process(self, ctx: ChatContext) -> None:
        ...
        for r in results:
            key = r.knowledge_id or "unknown"
            groups[key].append(r)

        merged: list[SearchResult] = []

        for kb_id, group in groups.items():
            # 按 chunk_index 排序
            sorted_group = sorted(group, key=lambda x: x.chunk_index or 0)

            for result in sorted_group:
                # 检查是否需要与前一个合并
                if merged:
                    last = merged[-1]
                    # 如果是相邻的 chunks 且内容较短，尝试合并
                    if (last.knowledge_id == result.knowledge_id and
                        last.chunk_index is not None and
                        result.chunk_index is not None and
                        last.chunk_index + 1 == result.chunk_index and
                        len(last.content) < self._min_content_length and
                        len(result.content) < self._min_content_length):
                        # 合并内容
                        merged_content = last.content + " " + result.content
                        ...
                        logger.info(
                            ...
                            chunk1=last.id,
                            chunk2=result.id,
                            combined_length=len(merged_content),
                        )
                        continue
        ...
        # 更新结果
        if ctx.rerank_results:
            ctx.rerank_results = merged
        else:
            ctx.search_results = merged
...
class EntityExtractStage:
    ...
    # 默认实体抽取提示词
    DEFAULT_EXTRACT_PROMPT = """你是一个实体抽取助手。请从用户查询中抽取以下类型的实体：
...
{
    "entities": [
        {"text": "实体文本", "type": "实体类型", "position": 起始位置}
    ],
    "keywords": ["关键词1", "关键词2"]
}
...
只输出 JSON，不要其他内容。"""
    ...
    def __init__(
        ...
        Args:
            llm_service: LLM 服务
            extract_prompt: 自定义抽取提示词
        """
        self._llm_service = llm_service
        self._extract_prompt = extract_prompt or self.DEFAULT_EXTRACT_PROMPT
    ...
    async def process(self, ctx: ChatContext) -> None:
        ...
        try:
            # 替换占位符
            prompt = self._extract_prompt.replace("{{query}}", ctx.query)
            ...
            try:
                ...
                # 存储抽取的实体
                ctx.config.extracted_entities = result.get("entities", [])
                ctx.config.extracted_keywords = result.get("keywords", [])

                logger.info(
                    "entity_extract_output",
                    session_id=ctx.config.session_id,
                    entity_count=len(ctx.config.extracted_entities),
                    keyword_count=len(ctx.config.extracted_keywords),
                )

            except json.JSONDecodeError as e:
                logger.warning(
                    "entity_extract_parse_failed",
                    session_id=ctx.config.session_id,
                    error=str(e),
...
class StreamFilterStage:
    """流式过滤阶段

    对齐 WeKnora99 PluginStreamFilter

    对流式响应中的关键内容进行实时过滤和标记。
    """
    ...
    def __init__(self, keywords: list[str] | None = None):
        ...
        Args:
            keywords: 需要标记的关键词列表
        """
        self._keywords = [k.lower() for k in keywords] if keywords else []
    ...
    async def process_stream(
        ...
        for keyword in self._keywords:
            if keyword.lower() in result.lower():
                # 可以在这里添加高亮标记
                # 例如: result = result.replace(keyword, f"**{keyword}**")
                pass
