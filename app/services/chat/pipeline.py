...
class ChatPipeline:
    ...
    使用示例:
        ```python
        from app.services.chat.pipeline import ChatPipeline, ChatPipelineConfig
    ...
    async def run_stream(self, query: str) -> AsyncGenerator[str]:
        """运行聊天管道（流式）
    ...
    async def _chat_completion_stream(self, ctx: ChatContext) -> AsyncGenerator[str]:
        """LLM 生成（流式）"""
        messages = self._build_messages(ctx)
    ...
    def _build_messages(self, ctx: ChatContext) -> list[dict[str, str]]:
        ...
        # 添加对话历史
        from app.services.chat.stages import remove_thinking_tags
