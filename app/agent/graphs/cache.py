...
提供 ChatGraph 实例的缓存机制，避免每次请求都重新编译图。
...
from app.agent.graphs.chat import ChatGraph
...
class GraphCache:
    ...
    def __init__(
        ...
        graph_factory: Callable[[str | None], ChatGraph] | None = None,
    ) -> None:
        """初始化图缓存
    ...
    def _default_graph_factory(self, system_prompt: str | None) -> ChatGraph:
        """默认图工厂函数
        ...
        Returns:
            编译后的 ChatGraph 实例
        """
        from app.agent.graphs.chat import create_chat_graph

        return create_chat_graph(system_prompt=system_prompt)
...
def get_cached_graph(system_prompt: str | None = None) -> CompiledStateGraph:
    ...
    Examples:
        ```python
        from app.agent.graphs.cache import get_cached_graph
