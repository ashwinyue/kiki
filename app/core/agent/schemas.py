...
class RouteDecision(BaseModel):
    ...
    agent: str = Field(
        description="目标 agent 的名称，必须从可用 agent 列表中选择"
    )
    reason: str = Field(
        description="选择该 agent 的原因，简要说明用户意图匹配"
    )
    confidence: float = Field(
        default=1.0,
        ge=0.0,
        le=1.0,
        description="决策的置信度，0.0 到 1.0"
    )
...
class SupervisorDecision(BaseModel):
    ...
    next: str = Field(
        description="下一个要执行的 worker 名称，或 'END' 表示结束"
    )
...
class ToolResult(BaseModel):
    ...
    result: str = Field(
        default="",
        description="工具执行结果或错误信息"
    )
    data: dict = Field(
        default_factory=dict,
        description="工具返回的额外数据"
    )
